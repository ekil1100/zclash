...
- [x] SOCKS5 代理
- [x] **混合端口** - HTTP + SOCKS5 共用一个端口（含 SOCKS5 握手）
...
启动时会自动验证配置，包含以下检查：
- ✅ 端口范围 (1-65535) 和冲突检测（`mixed-port` 开启时覆盖 `port`/`socks-port` 并给出 warning）
- ✅ `bind-address` 格式校验（`*` 或 IPv4）与 `allow-lan` 语义提示
- ✅ 代理节点必填字段验证
...
allow-lan: false
bind-address: "*"      # allow-lan=false 时会退回 127.0.0.1
mode: rule
log-level: info
