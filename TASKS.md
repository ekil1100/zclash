...
- [ ] P12-1A [1.1] 定义 P12 第一批任务
  - 范围：`TASKS.md`
  - DoD：产出 3-5 个原子任务（1.1 功能收尾或 1.2 规划），指定唯一 NEXT
  - 预估：0.5h

- [ ] P12-1B [1.0] 准备 v1.0.0 发布执行命令
  - 范围：`scripts/prepare-v1.0.0-release.sh`
  - DoD：脚本包含完整发布步骤（打 tag / push / 验证 release workflow / 更新 README 链接）
  - 注意：不实际执行，仅准备
  - 预估：0.5h

- [ ] P12-1C [1.1] 回归报告归档清理
  - 范围：`tools/config-migrator/reports/`
  - DoD：整理历史报告，保留最近 10 次 + latest，其余归档到 `history/`；不影响回归功能
  - 预估：0.5h

- [ ] P12-1D [1.1] 迁移规则文档补齐：规则速查表
  - 范围：`docs/compat/migrator-rules-quickref.md`
  - DoD：R1-R17 每条规则一句话说明 + 示例 + 修复建议
  - 预估：1h

- [ ] P12-1E [1.2] 1.2 规划草案（可选）
  - 范围：`docs/roadmap/1.2-planning.md`
  - DoD：基于 1.0/1.1 遗留项和锦上添花功能，列出 3-5 个候选方向
  - 预估：1h

- NEXT（唯一）：P12-1B（v1.0.0 发布准备，优先级最高）
