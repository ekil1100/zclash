...
| DNS 能力 | 解析策略与缓存体系成熟，故障场景经验丰富 | 基础 DNS 路径可用，策略/缓存可观测性仍弱 | DNS 抖动下的稳定性与诊断闭环不足 | 必补 | P0 | 需纳入回归门禁与阈值校验 |
| 观测与排障 | 指标、日志、事件链路较完整，定位路径成熟 | 有基础日志与状态，指标与事件关联不足 | 缺统一观测口径与故障定位指引 | 必补 | P0 | 需补指标-日志联动与排障手册 |
| 稳定性 | 长稳与异常恢复经验成熟 | 具备基础运行能力，长稳验证体系未完整 | 长稳测试、故障注入、回滚机制未成体系 | 必补 | P0 | 需纳入回归门禁 |
